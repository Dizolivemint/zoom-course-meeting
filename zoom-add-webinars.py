...
def read_csv():
    meetings = []
    with open('schedule_webinar.csv', newline='') as csvfile:
        ...
        for row in meeting_reader:
            meetings.append({
                ...
                'settings': {
                    ...
                    'approval_type': 0,
                    'registration_type': 1,
                    'alternative_hosts': 'mexner@pacificcollege.edu,lifelonglearning@pacificcollege.edu,scs-jose@pacificcollege.edu,tluger@pacificcollege.edu',
                    'contact_name': 'Pacific Symposium',
                    'contact_email': 'symposium@pacificcollege.edu',
                    'question_and_answer': {
                        'enabled': 'true',
...
def new_meeting_request(valid_jwt: str, meeting: dict) -> str:
    ...
    try:    
        conn.request('POST', f'/v2/users/{user_id}/webinars', body=json.dumps(meeting), headers=headers)
        res = conn.getresponse()
        data = json_to_dict(res)
    except Exception as x:
