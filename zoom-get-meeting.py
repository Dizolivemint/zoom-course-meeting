...
# Get Meeting
# ---Start--- #
def get_meeting_request(valid_jwt: str, meeting: dict, meeting_id: str) -> str:
    data = {}
    ...
    try:    
        conn.request('GET', f'/v2/meetings/{meeting_id}', body=None, headers=headers)
        res = conn.getresponse()
        print(res.status, res.read())
        data = json_to_dict(res)
    except Exception as x:
        data = { 'topic': f'Error {x} occured on call to get meeting' }
...
# Get Webinar
# ---Start--- #
def get_webinar_request(valid_jwt: str, meeting: dict, meeting_id: str) -> str:
    data = {}
    conn = http.client.HTTPSConnection('api.zoom.us')
    headers = {
		'authorization': f'Bearer {valid_jwt}',
    ...
    try:    
        conn.request('GET', f'/v2/webinars/{meeting_id}', body=None, headers=headers)
        res = conn.getresponse()
        print(res.status, res.read())
        data = json_to_dict(res)
    except Exception as x:
        data = { 'topic': f'Error {x} occured on call to get webinar' }
        
    # Error check if there is an issue finding the user
    if 'code' in data:
        data = { 'topic': f'Error: {data["message"]} occured on call to get webinar' }
...
if __name__ == '__main__':
    ...
    for body in meetings:
        ...
        if meeting == 'TRUE':
            print(f"Getting Meeting: {meeting_id} | {body['topic']}")
            data = get_meeting_request(new_jwt, body, meeting_id)
        else:
            print(f"Getting Webinar: {meeting_id} | {body['topic']}")
            data = get_webinar_request(new_jwt, body, meeting_id)
        ...
        log_csv = log_data(log_csv, meeting_id, data)
        
    if write_log(log_csv, f'get-log-{time.time()}.csv'):
